  ...
  for stack_i, stack in enumerate(STACKS):
    ...
    for cell in stack:
      if map[cell] == '.':
        empty_space = cell
        break
      if map[cell] != stack_ch: break
      distance_in -= 1
    if not empty_space: continue
    ...
    for cell in HALL_CELLS:
      if all((cell2 == cell and map[cell2] == stack_ch) or (cell2 != cell and map[cell2] == '.')
        for cell2 in range(min(cell, hall_entrance), max(cell, hall_entrance) + 1)):  # move home
