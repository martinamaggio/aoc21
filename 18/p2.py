...
def clone(x):
  if isinstance(x, Pair):
    return Pair(clone(x.left), clone(x.right))
  return Number(x.v)


numbers = []
for ll in lines:
  numbers.append(parse_number(eval(ll)))

answer = 0
values = range(len(numbers))
cycle = itertools.combinations(values, 2)

for n1, n2 in cycle:

  sum1 = Pair(clone(numbers[n1]), clone(numbers[n2]))
  reduce(sum1)
  mag1 = sum1.magnitude()
  if mag1 > answer:
    answer = mag1

  sum2 = Pair(clone(numbers[n2]), clone(numbers[n1]))
  reduce(sum2)
  mag2 = sum2.magnitude()
  if mag2 > answer:
    answer = mag2
